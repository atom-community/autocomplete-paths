...
export default class PathsProvider extends EventEmitter {
   ...
   */
  reloadScopes () {
    this._scopes = atom.config.get('autocomplete-paths-ignore-circle-symlinks.scopes').slice(0) || []

    if (!atom.config.get('autocomplete-paths-ignore-circle-symlinks.ignoreBuiltinScopes')) {
      this._scopes = this._scopes.concat(DefaultScopes)
    }

    for (var key in OptionScopes) {
      if (atom.config.get(`autocomplete-paths-ignore-circle-symlinks.${key}`)) {
  ...
  _getSuggestionsForScope (scope, request, match) {
    ...
    let suggestions = files.map(pathName => {
      const normalizeSlashes = atom.config.get('autocomplete-paths-ignore-circle-symlinks.normalizeSlashes')
  ...
  get suggestionPriority () {
    return atom.config.get('autocomplete-paths-ignore-circle-symlinks.suggestionPriority')
  }
