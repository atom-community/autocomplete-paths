...
export default class PathsProvider extends EventEmitter {
  ...
  _getSuggestionsForScope (scope, request, match) {
    ...
    const currentDirectory = this._getDirectoryForPath(request.editor.getPath())

    const requestedDirectoryPath = path.resolve(currentDirectory.path, parsedPathPrefix.dir)
   ...
   * @return {Directory}
   * @private
   * @TODO Can we improve performance by using strings as hash keys?
   */
  _getDirectoryForPath (filePath) {
    let directory = null
    this._pathsCache.getDirectories()
      .forEach(d => {
