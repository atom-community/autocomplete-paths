...
export default class PathsCache extends EventEmitter {
  ...
  _isPathIgnored (path) {
    ...
    if (atom.config.get('core.excludeVcsIgnoredPaths')) {
      this._repositories.forEach(repository => {
        if (ignored) return
        const ignoreSubmodules = atom.config.get('autocomplete-paths-ignore-circle-symlinks.ignoreSubmodules')
    ...
    if (atom.config.get('autocomplete-paths-ignore-circle-symlinks.ignoredNames')) {
    ...
    const ignoredPatterns = atom.config.get('autocomplete-paths-ignore-circle-symlinks.ignoredPatterns')
    if (ignoredPatterns) {
  ...
  _cacheDirectoryFilePaths (projectDirectory, directory) {
    ...
    return this._getDirectoryEntries(directory)
      .then(entries => {
        ...
        const maxFileCount = atom.config.get('autocomplete-paths-ignore-circle-symlinks.maxFileCount')
        if (newPathsCount > maxFileCount && !this._cancelled) {
          atom.notifications.addError('autocomplete-paths-ignore-circle-symlinks', {
            description: `Maximum file count of ${maxFileCount} has been exceeded. Path autocompletion will not work in this project.<br /><br /><a href="https://github.com/atom-community/autocomplete-paths-ignore-circle-symlinks/wiki/Troubleshooting#maximum-file-limit-exceeded">Click here to learn more.</a>`,
            dismissable: true
          })
  ...
  _populateCacheFor(projectDirectory) {
    ...
    const cmd = [
      'find',
      atom.config.get('autocomplete-paths-ignore-circle-symlinks.findOptions'), // default -L
      directoryPath + '/',
      '-type', 'f',
