...
export default class PathsCache extends EventEmitter {
  ...
  rebuildCache () {
    ...
    // check if find is available and store its path
    if (this.hasFind == null) {
      which('find')
      .then( (resolvedPath) => {
        this.findPath = resolvedPath
        this.hasFind = true
      })
      .catch( () => {
        this.hasFind = false
    ...
    try {
      if (this.hasFind) {
        // `find` is available
        return this._buildInitialCacheWithFind()
      } else {
  ...
  async _populateCacheFor(projectDirectory) {
    ...
    const files = _.compact(stdout.toString().split('\n'));
