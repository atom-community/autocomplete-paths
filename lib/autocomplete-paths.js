...
export default {
  ...
  _displayStatusBarTile () {
    ...
    if (!this._provider) { // TODO check why we need this check
      this.getProvider();
      if (!this._provider) {
        return;
      }
  ...
  _findAndLoadAliasConfig() {
    if (this._provider) {
      atom.project.getDirectories().forEach((projDir) => {
        /* TODO add weppack support
        var webpackConfig = (configPath && fs.existsSync(configPath)) ? require(configPath) : null;
        if (typeof webpackConfig === 'function') {
        webpackConfig = webpackConfig();
        }
        */
        let tsconf = projDir.getFile('tsconfig.json')
        if (tsconf.existsSync()) {
          try {
            tsconf = JSON.parse(tsconf.readSync());
            if (tsconf.compilerOptions && tsconf.compilerOptions.paths) {
              this._provider.setAliases(tsconf.compilerOptions.paths);
            }
          } catch(e) {
            console.error('autocomplete-paths: can\'t parse "tsconfig.json" for aliases. incorrect json')
          }
        }
      })
    }
  ...
  getProvider: function () {
    if (!this._provider) {
      ...
      atom.config.observe('autocomplete-paths.enableDetectAliases', (newValue, prev) => {
        if (newValue) {
          this._findAndLoadAliasConfig();
        } else {
          this._provider.clearAliases();
        }
      })
      if (atom.config.get('autocomplete-paths.enableDetectAliases')) {
        this._findAndLoadAliasConfig();
      }
    }
